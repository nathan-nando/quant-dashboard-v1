...
import { Grid, Column, Tile, Button, Toggle, TextInput, Select, SelectItem, Tag } from '@carbon/react';
...
function AccountContent() {
  ...
  useEffect(() => {
    ...
    // Fetch Trades
    fetch('http://localhost:8000/api/dashboard/trades')
      .then(res => res.json())
      .then(data => {
  ...
  return (
    <Grid fullWidth>
      <Column lg={16} md={8} sm={4} className="landing-page__banner" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <h3 style={{ marginBottom: "1rem", fontWeight: 400 }}>Account & Portfolio</h3>
        {accountData && accountData.metadata && (
          <div style={{ display: 'flex', gap: '0.5rem', marginBottom: "1rem" }}>
            <span style={{ padding: '4px 8px', backgroundColor: accountData.metadata.mode === 'LIVE' ? '#fa4d56' : '#0f62fe', color: 'white', borderRadius: '4px', fontSize: '12px', fontWeight: 600 }}>
              {accountData.metadata.mode}
            </span>
            <span style={{ padding: '4px 8px', backgroundColor: '#393939', color: '#c6c6c6', borderRadius: '4px', fontSize: '12px' }}>
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: '0.875rem' }}>
                      ...
                      <tbody>
                        {positions.map((p, idx) => {
                          ...
                          return (
                            <tr key={idx} style={{ borderBottom: '1px solid #393939' }}>
                              <td style={{ padding: '1rem', fontFamily: 'monospace' }}>#{p.ticket}</td>
